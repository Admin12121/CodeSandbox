...
def test_switching_tool_job_template_to_workflow_mode_with_empty_graph_does_not_500(ctx: TestContext) -> None:
    """Regression: switching interface_behavior to 'workflow' before a graph
    has been built used to hardcode the fallback ui_mode to terminal_only,
    which validate_ui_mode_config rejects for tool_job (not terminal-capable)
    — save_template raised instead of returning a clean error tuple. Every
    runtime_class must fall back to *its own* default mode, not a fixed one."""
    from codesandbox.features.sandbox.service import save_template
    ...
    assert error is None, error
    assert result["default_ui_mode"] == "background_run"
    assert result["allowed_ui_mode_values"] == ["background_run"]
...
TESTS: list[TestCase] = [
    ...
    TestCase("switching tool_job template to workflow mode with empty graph does not 500", "sandbox", test_switching_tool_job_template_to_workflow_mode_with_empty_graph_does_not_500),
